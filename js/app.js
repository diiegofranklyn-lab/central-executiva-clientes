...
.brand-logo{height:76px;display:flex;align-items:center;justify-content:flex-start;overflow:visible}
.brand-logo>img.security-logo{display:block!important;width:190px;height:74px;object-fit:contain;object-position:left center;filter:none!important;margin:0}
.brand-logo>span,.brand-logo{font-size:0}.brand-logo:before,.brand-logo:after{display:none!important}
.brand-sub{font-size:8px;letter-spacing:.14em;margin-top:2px;color:#fff;font-weight:700}
...
export function setupShell(active){applyModernUI();const brandLogo=document.querySelector('.brand-logo');if(brandLogo){brandLogo.innerHTML='';const img=document.createElement('img');img.className='security-logo';img.src='./assets/logo-security-original.svg?v=5';img.alt='Security — Segurança e Serviços';brandLogo.appendChild(img);}document.querySelectorAll('.nav a').forEach(a=>{a.classList.toggle('active',a.dataset.page===active);const icon=a.querySelector('.icon');if(icon)icon.textContent=icons[a.dataset.page]||'';});const btn=$('logoutButton');if(btn)btn.addEventListener('click',async()=>{await supabase.auth.signOut();window.location.href='./index.html';});}
