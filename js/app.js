...
.nav .icon{width:20px;height:20px;flex-basis:20px;display:grid;place-items:center;position:relative;color:#fff;font-size:0;line-height:1}.nav .icon:before{display:none!important}.nav .icon:after{display:none!important}
.nav a[data-page="dashboard"] .icon{font-size:16px}.nav a[data-page="dashboard"] .icon::before{content:'▦';display:block!important;font-size:16px}.nav a[data-page="solicitacoes"] .icon::before{content:'▤';display:block!important;font-size:16px}.nav a[data-page="clientes"] .icon::before{content:'●';display:block!important;font-size:14px}.nav a[data-page="relatorios"] .icon::before{content:'▥';display:block!important;font-size:16px}
...
@media(max-width:700px){.sidebar{width:70px;padding:14px 8px}.brand-logo{height:45px;justify-content:center}.brand-logo>img.security-logo{display:none!important}.brand-logo:after{content:'S';font-size:26px;font-weight:800;color:#fff}.brand-sub{display:none}.nav{padding-top:15px}.nav a{justify-content:center;padding:11px 8px}.nav a.active{box-shadow:none}.main{margin-left:70px;width:calc(100% - 70px)}.top-header{height:58px;padding:0 14px}.content{padding:18px 13px 30px}.profile{padding:5px}.profile .profile-text{display:none}}
`;document.head.appendChild(style);}

export function setupShell(active){applyModernUI();const brandLogo=document.querySelector('.brand-logo');if(brandLogo){brandLogo.innerHTML='';const img=document.createElement('img');img.className='security-logo';img.src='./assets/logo-security-original.svg?v=3';img.alt='Security — Segurança e Serviços';brandLogo.appendChild(img);}document.querySelectorAll('.nav a').forEach(a=>{a.classList.toggle('active',a.dataset.page===active);const icon=a.querySelector('.icon');if(icon)icon.textContent='';});const btn=$('logoutButton');if(btn)btn.addEventListener('click',async()=>{await supabase.auth.signOut();window.location.href='./index.html';});}
